...
same_count_zip_2017 = counts_2017_2018[(counts_2017_2018['ZIP_CD'].isin(closed_zip_2017)) & (counts_2017_2018[2017] < counts_2017_2018[2018])]
...
# filter for ZIP codes in closed_zip_2017 with less count in 2017 than 2018
same_count_zip_2018 = counts_2018_2019[(counts_2018_2019['ZIP_CD'].isin(closed_zip_2018)) & (counts_2018_2019 [2018] < counts_2018_2019[2019])]

same_count_zip_2018

same_count_zip = combined_df = pd.concat([same_count_zip_2017, same_count_zip_2018], ignore_index=True)
```

    a. How many hospitals fit this definition of potentially being a merger/acquisition?
```{python}

print(f"{same_count_zip.shape[0]} hospitals fit this definition of potentially being a merger/acquisition.")
```

    b. After correcting for this, how many hospitals do you have left?
```{python}
# filter merger hospitials
closed_hospitals = closed_hospitals[~closed_hospitals['ZIP_CD'].isin(same_count_zip['ZIP_CD'])]
